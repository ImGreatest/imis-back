...
export class AuthTokenService {
	...
	async generateRefreshToken(userId: number, deviceId: string): Promise<string> {
		const token = await this.prisma.refreshToken.create({
			data: {
				token: randomBytes(config.RefreshLength).toString('hex'),
				deviceId: deviceId,
				userId: userId,
				expieres_at: DateTime.now()
					.plus({ month: 1 })
					.toJSDate()
			}
		});
	...
	async refresh(data: ReqRefreshDto): Promise<ResSignInDto> {
		...
		const roleUser: number = await this.userService.getUserRoleId(oldRefresh.userId);
		...
		const refresh: string = await this.generateRefreshToken(oldRefresh.userId, oldRefresh.deviceId);
